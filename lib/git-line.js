...
const parseOrigin = () => {
  const originUrl = atom.project.getRepositories()[0].getOriginURL().replace('.git', '');
  ...
  // so we only need to remove the .git
  return originUrl;
}
...
export default {
  ...
  message(output) {
    ...
    const serviceName = isGithub ? 'Github' : 'Bitbucket';
